...
var BareMultiSigHashInput = Input.BareMultiSigHash;
...
Transaction.prototype.fromObject = function(transaction) {
  ...
  _.each(transaction.inputs, function(input) {
    ...
    } else if (input.output.script.isMultisigOut() && input.publicKeys && input.threshold) {
      txin = new Input.BareMultiSigHash(
        input, input.publicKeys, input.threshold, input.signatures
      );
    } else {
...
Transaction.prototype.from = function(utxo, pubkeys, threshold) {
  ...
  if (utxo.script && utxo.script.isMultisigOut && utxo.script.isMultisigOut()) {
    this._fromBareMultisigUtxo(utxo, pubkeys, threshold);
  } else if (pubkeys && threshold) {
    this._fromMultisigUtxo(utxo, pubkeys, threshold);
  } else {
    this._fromNonMultisig(utxo);
  }
  return this;
};

Transaction.prototype._fromNonMultisig = function(utxo) {
  var clazz;
...
Transaction.prototype._fromBareMultisigUtxo = function(utxo, pubkeys, threshold) {
  $.checkArgument(threshold <= pubkeys.length,
    'Number of required signatures must be greater than the number of public keys');
  utxo = new UnspentOutput(utxo);
  this.addInput(new BareMultiSigHashInput({
    output: new Output({
      script: utxo.script,
      satoshis: utxo.satoshis
    }),
